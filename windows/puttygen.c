...
void modalfatalbox(const char *fmt, ...)
{
    ...
    va_end(ap);
    MessageBox(NULL, stuff, "PuTTYgen 致命错误",
...
void nonfatal(const char *fmt, ...)
{
    ...
    stuff = dupvprintf(fmt, ap);
    va_end(ap);
    MessageBox(NULL, stuff, "PuTTYgen 错误",
               MB_SYSTEMMODAL | MB_ICONERROR | MB_OK);
    sfree(stuff);
}
                   ...
                   Filename *filename, bool was_import_cmd)
{
    ...
    if (comment)
        sfree(comment);
    if (ret == 0) {
        char *msg = dupprintf("无法载入私钥 (%s)", errmsg);
        message_box(hwnd, msg, "PuTTYgen 错误", MB_OK | MB_ICONERROR,
                    false, HELPCTXID(errors_cantloadkey));
        sfree(msg);
    } else if (ret == 1) {
    ...
    switch (msg) {
      case WM_INITDIALOG:
        ...
        SetWindowLongPtr(hwnd, GWLP_USERDATA, (LONG_PTR) state);
        {
            ...
            menu1 = CreateMenu();
            AppendMenu(menu1, MF_ENABLED, IDC_LOAD, "载入私钥(&L)");
...

        {
            ...
            cp2 = cp;
            statictext(&cp2, "没有密钥。", 1, IDC_NOKEY);
            cp2 = cp;
            ...
            staticbtn(&cp, "生成公私密钥对",
                      IDC_GENSTATIC, "生成(&G)", IDC_GENERATE);
            staticbtn(&cp, "载入私钥文件",
                      IDC_LOADSTATIC, "载入(&L)", IDC_LOAD);
      ...
      case WM_COMMAND:
        switch (LOWORD(wParam)) {
          ...
          case IDC_EXPORT_SSHCOM:
            ...
            if (state->key_exists) {
                ...
                Filename *fn = request_file(
                    hwnd, "保存私钥为：", NULL, true, NULL, false,
                ...
                if (fn) {
                    ...
                    if (ret <= 0) {
                        MessageBox(hwnd, err ? err : "无法保存密钥文件",
                                   "PuTTYgen 错误", MB_OK | MB_ICONERROR);
                    }
                    sfree(err);
                    filename_free(fn);
          ...
          case IDC_SAVEPUB:
            ...
            if (state->key_exists) {
                Filename *fn = request_file(
                    hwnd, "保存公钥为：", NULL, true, NULL, false,
                    FILTER_ALL_FILES);
                if (fn) {
                    int ret;
                    FILE *fp = f_open(fn, "r", false);
          ...
          case IDC_IMPORT:
            ...
            if (!state->generation_thread_exists) {
                Filename *fn = request_file(
                    hwnd, "载入私钥：", NULL, false, NULL, false,
          ...
          case IDC_ADDCERT:
            ...
            if (state->key_exists && !state->generation_thread_exists) {
                Filename *fn = request_file(
                    hwnd, "载入证书：", NULL, false, NULL, false,
                    FILTER_ALL_FILES);
                if (fn) {
...
static NORETURN void opt_error(const char *fmt, ...)
{
    ...
    MessageBox(NULL, msg, "PuTTYgen 命令行错误", MB_ICONERROR | MB_OK);
